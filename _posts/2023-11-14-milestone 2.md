...
As seen from the above graphs these 3 Logistic Regression Models performed similar to the validation set. There isnt any drastic change in the graph except for some slight movement in the goal rate of net_distance. The models were expected to behave the same on test set as on the validation set since these logistic regression models are simple as they use only 1 to max 2 features to predict the outcome. 
