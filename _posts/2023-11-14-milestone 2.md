...
So, conclusively we can say that the model is generalizable well on the test set, since the performance is similar to the validation set and the ROC curve is consistent for both the datasets. Though a clear exception to the Reliability Curve is the XGBoost model which is not able to predict the probability of the goal properly on the Regular Dataset but does fine on Playoff Dataset. 
