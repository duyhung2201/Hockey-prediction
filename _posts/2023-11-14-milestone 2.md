...
### Logistic Regression Models on Regular Dataset

![Alt text](../assets/milestone2/Question7/Regular_net_distance.png)

[link](https://www.comet.com/duyhung2201/ift-6758-b03-project/addcd095d483415bbf9f9d721fec6a9d?experiment-tab=panels&showOutliers=true&smoothing=0&xAxis=step)

![Alt text](../assets/milestone2/Question7/regular_shot_angle.png)

[link](https://www.comet.com/duyhung2201/ift-6758-b03-project/d29190c6a8844b1a912709145ffa225c?experiment-tab=panels&showOutliers=true&smoothing=0&xAxis=step)

![Alt text](../assets/milestone2/Question7/regular_angle_distance.png)

[link](https://www.comet.com/duyhung2201/ift-6758-b03-project/769a8821c5964d68ab7797b02e7bbc63?experiment-tab=panels&showOutliers=true&smoothing=0&xAxis=step)

As seen from the above graphs these 3 Logistic Regression Models performed similar to the validation set.

The ROC curve for the net_disance on the regular test set is 0.69 which is similar to the validation set. The goal rate has a very minute differnece in the regular test set as the highest predicted probability starts around 25% of goal rate whereas in the validation set it was almost close to 30% but both of the  set follows the same trend of decreasing as the percentile decreases. The cumulative proportion of goals and reliability diagram is exactly the same.   

As for the shot_angle, there isnt any difference between the AUC score, goal rate, cumulative proportion of goals and reliability diagram. The performance on these graphs are not good in both the validation and regular test set.

finally for the shot_angle and net_distance we can can see that the graphs are similar to the net_distance.This was the case for the validation set as well showing that the shot_angle does not have much importance.  

There isn't any drastic change in the graph except for some slight movement in the goal rate of net_distance. The models were expected to behave the same on test set as on the validation set since these logistic regression models are simple as they use only 1 to max 2 features to predict the outcome. We want the models have similaer performance on validation and test set since we would like the model to generalise better on the unseen data

### Logistic Regression on Playoff Dataset

![Alt text](../assets/milestone2/Question7/playoff_net_distance.png)

[link](https://www.comet.com/duyhung2201/ift-6758-b03-project/0b45540a22bd485b8d90883459f90474?experiment-tab=panels&showOutliers=true&smoothing=0&xAxis=step)

![Alt text](../assets/milestone2/Question7/playoff_shot_angle.png)

[link](https://www.comet.com/duyhung2201/ift-6758-b03-project/bf4915a545934bb1b66667759d9442a8?experiment-tab=panels&showOutliers=true&smoothing=0&xAxis=step)

![Alt text](../assets/milestone2/Question7/playoff_angle_distance.png)

[link](https://www.comet.com/duyhung2201/ift-6758-b03-project/61f6f8d8a97449dd932df948267447fd?experiment-tab=panels&showOutliers=true&smoothing=0&xAxis=step)

As seen from the above graphs these 3 Logistic regression follows a similar trend as to the validation set and regulart test set. 

The ROC curve for net_distance is 0.68 which has slightly decreased when compared to the validation set and test set. The  goal rate for the playoff test set starts from 20% for the highest predicted probability then slightly decreases and goes up again and then has a gradual decrease as the model percentile decreases. This type of trend in goal rate is different than of the previous test set and validation set. The reliability diagram is also differnt since the the classifier jumps below the "Perfectly Calibrated" line at the point close to 0.2.

As for the shot__angle the graph is very similar to the other test case and the validation set. The goal rate has a slightly different trend as it starts from 0% of goal rate and has a steady line as the percentile increases. 

For the shot_angle and net_distance is very similar to the graph of net_distance. This was the case for the validation set and regular test set. 

Overall these graphs shows a similar trend to the validation set except for some slight changes in AUC score and goal rate. 
