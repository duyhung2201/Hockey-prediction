...
As for the shot_angle, there isnt any differnce between the AUC score, goal rate, cumulative proportion of goals and reliability diagram. The performance on these graphs are not good in both the validation and regular test set.

finally for the shot_angle and net_distance we can can see that the graphs are similar to the net_distance.This was the case for the validation set as well showing that the angle_angle does not have much importance.  
